...
def train(data_folder: pathlib.Path,
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=TRAINING_BATCH_SIZE, shuffle=True, collate_fn=collate_fn_slice_add
    )
    
    #print(f"Train 로더 크기는?: {len(train_dataloader)}")
    
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=TRAINING_VAL_BATCH_SIZE, shuffle=False, collate_fn=collate_fn_slice_add
    )
