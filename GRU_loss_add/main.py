...
def main(data_folder: pathlib.Path, model_folder: pathlib.Path, verbose: int, num_k, val_fold_num, device):
    ...
    val_features, _, _, _, = batch_whole_dataset(val_dataset)
...
def train(data_folder: pathlib.Path,
    ...
    class_weights_train_frame = calculate_class_weights_wo(train_dataset)
    
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=TRAINING_BATCH_SIZE, shuffle=True, collate_fn=collate_fn_multi_output
    )
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=TRAINING_VAL_BATCH_SIZE, shuffle=False, collate_fn=collate_fn_multi_output
    )
    ...
    crit_murmur = nn.CrossEntropyLoss()
    crit_outcome = nn.CrossEntropyLoss()
    ...
    for epoch in tqdm(range(TRAINING_MAX_EPOCHS)):
        ...
        for i, (bs_features, bs_frame_labels, bs_murmur_labels, bs_outcome_labels) in enumerate(train_dataloader):
            ...
            total_loss.backward()
            optim.step()
            running_loss += total_loss.item() * train_batch_size
        ...
        with torch.no_grad():
            ...
            for i, (bs_features, bs_frame_labels, bs_murmur_labels, bs_outcome_labels) in enumerate(val_dataloader):
                ...
                val_losses += total_loss.item() * val_batch_size
        ...
        # if verbose:
        #     print(
        #         f"\r{epoch +1 :04d}/{TRAINING_MAX_EPOCHS} | {loss :.3f} | {early_stop_counter:02d}" )
    ...
    loss_df = pd.DataFrame({'Epoch': range(1, epoch + 2),
