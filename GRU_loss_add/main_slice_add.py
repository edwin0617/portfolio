...
def main(data_folder: pathlib.Path, model_folder: pathlib.Path, verbose: int, num_k, val_fold_num, device):
    ...
    for filename, val_feature in zip(val_results_df.index, val_features):
        ...
        frame_preds, murmur_preds, outcome_preds = model(val_feature)
        
        #print(frame_preds.shape) #[1, 5, 300]
        frame_preds =  frame_preds.squeeze()
        outcome_preds =  outcome_preds.squeeze()
        
        
        print("@@@@@@@@@@@@@@@@@@@@")
        print(frame_preds[-1, :].mean().item())
        print("@@@@@@@@@@@@@@@@@@@@")
        
        
        val_results[filename]= {"holo_HSMM_mean": frame_preds[-1, :].mean().item(),
                                "holo_HSMM_max": frame_preds[-1, :].max().item(),
                                 }
    ...
    merged_df = merge_df_2(val_results_df, patient_df)
...
def train(data_folder: pathlib.Path,
    ...
    class_weights_train_murmur = None #torch.tensor([1.0, 10.0])
...
############################__Please assign PATH, GPU_index__########################################################################################
    ...
    args.model_name = 'BCE_two_loss_softmax'
