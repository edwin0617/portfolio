...
def main(configs, data_folder: pathlib.Path, test_data_folder: pathlib.Path, external_data_folder: pathlib.Path,  model_folder: pathlib.Path, verbose: int, num_k, val_fold_num, device):
    ...
    test_murmur_score = test_with_packed(configs, test_data_folder, model_folder, optim_threshold, fold, split_ratio, model, device)
...
def train_and_validate_model(configs, 
    ...
    for epoch in range(1, train_max_epochs + 1):
        ...
        for i, (train_mels, train_seq_labels, train_actual_lengths, train_mm_labels, _) in enumerate(train_dataloader):
            ...
            if torch.isnan(train_mels).any() or torch.isinf(train_mels).any():
                print("\nmel 전처리가 문제다 ㅎㅎㅎㅎ.......\n")
                
            if torch.isnan(train_seq_labels).any() or torch.isinf(train_seq_labels).any():
                print("\nSeq 전처리가 문제다 ㅎㅎㅎㅎ.......\n")
                
            if torch.isnan(train_mm_labels).any() or torch.isinf(train_mm_labels).any():
                print("\nMurmur 전처리가 문제다 ㅎㅎㅎㅎ.......\n")                      
                
            # print(f"\ni\n")
            # print(train_mels.shape, train_seq_labels.shape, train_actual_lengths.shape, train_mm_labels.shape)
        ...
        with torch.no_grad():
            ...
            if (val_seq_loss_all < best_seq_loss) & (val_murmur_loss_all < best_murmur_loss):
                ...
                early_stop_counter = 0
                print(f"Model parameters updated in epoch {epoch}")
            else:
...
def predict_single_model_packed(configs, data_folder, dataframe, model, device):
    ...
    infer_dataset = DatasetFor_Inference(data_folder, 
                                ...
                                sampling_rate= configs["data_preprocess"]["sampling_rate"],
                                cache={})
    mels, _ , actual_lengths, _, filenames= batch_whole_dataset_inmain(infer_dataset)
