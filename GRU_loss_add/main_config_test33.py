...
def train(configs, 
    ...
    stronglabeled_df = stronglabeled_df.sample(frac=1)
    ...
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size= configs["train_params"]["val_batch_size"], shuffle=False, collate_fn=collate_fn_slice_clean
    )
    
    
    for epoch in range(1, train_max_epochs + 1):
                
        train_loader, dataset_nums, bs_nums = get_trainloader(configs, external_data_folder,
                strong_data_folder, stronglabeled_df[600 * (epoch % 3) : 600 * (epoch % 3 +1)],
                weaklabeled_df, unlabeled_df)
        
        strong_bs, weak_bs, unlabeled_bs = bs_nums
        ...
        for i, (train_mels, train_seq_labels, train_mm_labels, _, _) in enumerate(train_loader):
            ...
            # seq_loss
            ## seq_loss = F.binary_cross_entropy(seq_preds_stu[:mask_strong], train_seq_labels[:mask_strong], reduction= 'none')
            ## train_seq_mask = train_seq_labels[:mask_strong] != -1 # Remove non-sound part
            ## filt_seq_loss = seq_loss[train_seq_mask].mean()
            
            # seq_loss
            seq_loss = crit_seq(F.softmax(seq_preds_stu[mask_strong], dim= 1), 
                                train_seq_labels[mask_strong])
            
            #print(seq_preds_stu[mask_strong].shape, train_seq_labels[mask_strong].shape)
            # torch.Size([82, 5, 298]) torch.Size([82, 5, 298])
            
            # murmur_loss
            murmur_loss = crit_murmur(F.softmax(murmur_preds_stu[mask_weak], dim= -1), train_mm_labels[mask_weak])
            
            #print(murmur_preds_stu[mask_weak].shape, train_mm_labels[mask_weak].shape)
            # torch.Size([94, 2]) torch.Size([94, 2])
            ...
            # /Data1/hmd2/notebooks_th/GRU_loss_add/models/2024-08-15_17:09:47
            # total_loss = seq_loss + w_weak * murmur_loss + \
            #     w_cons * (cons_seq_loss + w_weak_cons * cons_murmur_loss)
        ...
        train_losses.append(running_loss / sum(dataset_nums))
        train_seq_losses.append(running_seq_loss / dataset_nums[0]) # strong_nums
        train_murmur_losses.append(running_murmur_loss / sum(dataset_nums[:2])) # strong_nums + weak_nums
        train_cons_seq_losses.append(running_cons_seq_loss /  sum(dataset_nums))
        train_cons_murmur_losses.append(running_cons_murmur_loss /  sum(dataset_nums))
        ...
        with torch.no_grad():
            ...
            for i, (val_features, val_seq_labels, val_murmur_labels, _, _) in enumerate(val_dataloader):
                ...
                seq_loss_stu = crit_seq(F.softmax(seq_preds_stu, dim= 1), val_seq_labels)
                seq_loss_tch = crit_seq(F.softmax(seq_preds_tch, dim= 1), val_seq_labels)
                
                murmur_loss_stu = crit_murmur(F.softmax(murmur_preds_stu, dim= -1), val_murmur_labels)
                murmur_loss_tch = crit_murmur(F.softmax(murmur_preds_tch, dim= -1), val_murmur_labels)
        ...
        if verbose >= 1:
            print(f"epoch: {epoch:04d}/{train_max_epochs}\n \
                Train_loss: {running_loss / sum(dataset_nums):.4f}\n \
                Train_seq_loss: {running_seq_loss / dataset_nums[0]:.4f}\n \
                Train_murmur_loss: {running_murmur_loss / sum(dataset_nums[:2]):.4f}\n \
                Train_seq_cons_loss: {running_cons_seq_loss / sum(dataset_nums):.4f}\n \
                Train_murmur_cons_loss: {running_cons_murmur_loss / sum(dataset_nums):.4f}\n \
...
def get_trainloader(configs,
    ...
    strong_nums = stronglabeled_df.shape[0] - (stronglabeled_df.shape[0] % strong_bs)
    weak_nums = weaklabeled_df.shape[0] - (weaklabeled_df.shape[0] % weak_bs)
    unlabel_nums = unlabeled_df.shape[0] - (unlabeled_df.shape[0] % unlabeled_bs)
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_sampler=train_batch_sampler, num_workers= 0 , collate_fn= collate_fn_slice_clean)

    return train_dataloader, [strong_nums, weak_nums, unlabel_nums], (strong_bs, weak_bs, unlabeled_bs)
