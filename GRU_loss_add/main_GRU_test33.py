...
def main(configs, data_folder: pathlib.Path, test_data_folder: pathlib.Path, external_data_folder: pathlib.Path,  model_folder: pathlib.Path, verbose: int, num_k, val_fold_num, device):
    ...
    # Prediction of Good quality audio in i'th fold
    for k, v in val_fold_results.items():
        val_results[k].append(v)
    
    # Prediction of Bad quality audio in i'th fold
    unknown_posteriors = predict_single_model_packed(configs, data_folder, recording_df_bq, model, device)  
    ...
    test_murmur_score = test_with_packed(configs, test_data_folder, model_folder, optim_threshold, split_ratio, model, device)
...
def test_with_packed(configs, 
        test_data_folder: pathlib.Path, model_folder: pathlib.Path, 
        optim_threshold: float, split_ratio: float,
        model, device):
    ...
    infer_dataset = DatasetFor_Inference(test_data_folder, 
                                ...
                                cache={})
    test_dataloader = torch.utils.data.DataLoader(infer_dataset, batch_size= 1, shuffle=False, collate_fn= collate_fn_inmain)
    ...
    for idx, (mel, _, actual_lengths, _, filename) in tqdm(enumerate(test_dataloader)):    
        mel, model = map(lambda x: x.to(device), [mel, model])
        murmur_pred_per_fold = []
        
        with torch.no_grad():
            model = load_model_inmain(model_folder, model_name).to(device)
            _, murmur_pred = model(mel, actual_lengths)
            
            # print(murmur_pred.shape)
            
            murmur_pred_per_fold.append(murmur_pred.squeeze(0)[1].item())
        
        murmur_pred_ensemble = np.mean(murmur_pred_per_fold).item()    

        # Tuple, (filename)
        test_murmur_preds_per_file[filename[0].split(".wav")[0]] = {"holo_HSMM": murmur_pred_ensemble}
        
        print(f"Inferencing {filename}.wav is Done!, Total completed test file= {idx+1}", end="")
...
def train_and_validate_model(configs, 
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size= configs["train_params"]["strong_batch_size"], shuffle=True, collate_fn= collate_fn_inmain
    )
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size= configs["train_params"]["val_batch_size"], shuffle=False, collate_fn= collate_fn_inmain
    )
    ...
    for epoch in range(1, train_max_epochs + 1):
        ...
        for i, (train_mels, train_seq_labels, train_actual_lengths, train_mm_labels, _) in enumerate(train_dataloader):
            
            train_bs = train_mels.shape[0]
            
            train_mels, train_seq_labels, train_mm_labels = map(lambda x: x.to(device), [train_mels, train_seq_labels, train_mm_labels])
            
            optim.zero_grad()
            
            train_seq_preds, train_murmur_preds = model(train_mels, train_actual_lengths)
        ...
        with torch.no_grad():
            ...
            for i, (val_mels, val_seq_labels, val_actual_lengths, val_mm_labels, _) in enumerate(val_dataloader):
                
                val_bs = val_mels.shape[0]
                val_mels, val_seq_labels, val_mm_labels = map(lambda x: x.to(device), [val_mels, val_seq_labels, val_mm_labels])
                val_seq_preds, val_murmur_preds = model(val_mels, val_actual_lengths)
                
                seq_loss = crit_seq(val_seq_preds, val_seq_labels)
                murmur_loss = crit_murmur(val_murmur_preds, val_mm_labels)
                total_loss = seq_loss + murmur_loss
            ...
            if verbose >= 1:
                    print(f"epoch: {epoch:04d}/{train_max_epochs}",
                        f"Train_total_loss: {running_total_loss / len(train_dataset):.5f}", 
                        f"Val_total_loss: {val_total_loss_all:.5f}", 
                        f"Train_Seq_loss: {running_seq_loss / len(train_dataset):.5f}", 
                        f"Val_Seq_loss: {val_seq_loss_all:.5f}",
                        f"Train_murmur_loss: {running_murmur_loss / len(train_dataset):.5f}",
                        f"Val_murmur_loss: {val_murmur_loss_all:.5f}",
                        end= "")

            if (val_seq_loss_all < best_seq_loss) & (val_murmur_loss_all < best_murmur_loss):
                save_model_inmain(model_folder, model)
                best_seq_loss = val_seq_loss_all
                best_murmur_loss = val_murmur_loss_all
                early_stop_counter = 0
                print(f"\nModel parameters updated in epoch {epoch}\n")
            else:
    ...
    ith_loss_df.to_csv(model_folder / 'dataframes' /  f"loss_df.csv")      
    ...
    fold_posteriors = predict_single_model_packed(configs, strong_data_folder, validation_df, model, device)        
...
def predict_single_model_packed(configs, data_folder, dataframe, model, device):
    ...
    mels, _ , actual_lengths, _, _ = batch_whole_dataset_inmain(infer_dataset)
    mels, model = map(lambda x: x.to(device), [mels, model])
    
    with torch.no_grad():
        _, murmur_preds = model(mels, actual_lengths)
        results = murmur_preds[:, 1].cpu().tolist()

        return {n: p for n, p in zip(dataframe.index, results)}

def batch_whole_dataset_inmain(dataset: RecordingDataset):
    num_examples = len(dataset)
    return collate_fn_inmain([dataset[i] for i in range(num_examples)]) 

# Load your trained model.            
def save_model_inmain(model_folder: pathlib.Path, model: nn.Module):
    model_name = type(model).__name__
    model_path = model_folder / f"{model_name}.pt"
    torch.save(model.state_dict(), model_path)

# Load your trained model.
def load_model_inmain(model_folder: pathlib.Path, model_name: str):
...
def collate_fn_inmain(x):
    ...
    # feature, segmentation_label, murmur_label, outcome_label, filename

    for features, seq_label, murmur_label, _, filename in x:
                
        all_features.append(features)
        seq_labels.append(seq_label)
        max_length = max(max_length, seq_label.shape[-1])
        actual_lengths.append(seq_label.shape[-1])
        murmur_labels.append(murmur_label)
    ...
    padded_labels = seq_labels[0].new_full((len(x), seq_labels[0].shape[0], max_length), 0.0)

    
    for i, seq_label in enumerate(seq_labels):
        padded_labels[i, ..., : seq_label.shape[-1]] = seq_label

    return padded_features, padded_labels, torch.as_tensor(actual_lengths), torch.stack(murmur_labels), filenames
...
######################################################################################################################################################    
    ...
    configs = main(configs = configs, data_folder= args.train_data, test_data_folder= args.test_data, external_data_folder= args.external_data , 
                   model_folder= model_folder, verbose= args.verbose, num_k= 4, val_fold_num= 3, device= device)
