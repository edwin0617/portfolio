...
from dataset import collate_fn, calculate_features, merge_df
from data_augment import mixup_strong
from model import MHA_LSTM_ln, MHA_LSTM_simpler
from metric import*
...
def train_validate_model(config, df_dict, train_dataset, unlabeled_dataset, val_dataset):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size= config.exp_settings.exp_params.strong_bs, shuffle=True, collate_fn= collate_fn, num_workers=config.virtual_settings.num_workers)
    unlabeled_dataloader = DataLoader(unlabeled_dataset, batch_size= config.exp_settings.exp_params.unlabel_bs, sampler= sampler_unlabeled, collate_fn= collate_fn, num_workers=config.virtual_settings.num_workers)
    val_dataloader = DataLoader(val_dataset, batch_size= config.exp_settings.exp_params.val_bs, shuffle=False,  collate_fn= collate_fn, num_workers=config.virtual_settings.num_workers) 
    
    # Student & Teacher
    student = MHA_LSTM_simpler().cuda()
    ...
    optimizer = torch.optim.AdamW(student.parameters(), 
                            lr= config.exp_settings.exp_params.learning_rate,
                            weight_decay= config.exp_settings.exp_params.weight_decay, 
                            betas= config.exp_settings.exp_params.betas, 
                            eps=1e-8)
    
    
    consis_weight = config.exp_settings.mean_teacher.const_max
    
    best_total_loss, best_seq_loss, best_murmur_loss= tuple([np.inf] * 3)
    early_stop_counter, optim_count = 0, 0
    
    pos_weight = torch.tensor([1.0, config.exp_settings.exp_params.pos_weight]).float().cuda()
    crit_murmur = nn.CrossEntropyLoss(weight = pos_weight)
    
    # per epoch
    for epoch in tqdm(range(1, config.exp_settings.exp_params.total_epoch + 1)):
        student.train()
        ...
        for i, (batch_strong, batch_unlabel) in enumerate(zip(train_dataloader, cycle(unlabeled_dataloader))):
            ...
            # concat label & unlabel batch data
            ...
            if config.exp_settings.exp_params.use_mix_up:
            ...
            # Murmur pred loss
            mm_loss = crit_murmur(mm_pred_stu[label_mask], batch_mm_labels[label_mask])
            ...
            total_loss = (seq_loss + mm_loss)  +  cons_loss * consis_weight
            ...
            train_mm_losses = mm_loss.cpu().item() # * batch_num_strong
            ...
            # Update EMA
            config, student, teacher = update_ema(config, epoch, optim_count,
                                        student, teacher)
        ...
        with torch.no_grad():
            ...
            # if val_total_losses < best_total_loss:
                ...
                if early_stop_counter > config.exp_settings.exp_params.training_patience:
                    break
...
def train_model_full(config, stop_epoch, train_dataset, unlabeled_dataset):
    ...
    # per epoch
    for epoch in tqdm(range(1, stop_epoch + 1)):
        student.train()
        current_lr = optimizer.param_groups[0]['lr']
        consis_weight *= 0.99
        ...
        for i, (batch_strong, batch_unlabel) in enumerate(zip(train_dataloader, cycle(unlabeled_dataloader))):
            ...
            batch_num = batch_mels.shape[0]
            label_mask = torch.zeros(batch_num)
            unlabel_mask = torch.zeros(batch_num)
            
            label_mask[ : la_mels.shape[0]] = 1
            unlabel_mask[la_mels.shape[0] : ] = 1  
            
            label_mask = label_mask.bool()
            unlabel_mask = unlabel_mask.bool()
            ...
            # Consistency loss
            cons_seq_loss = F.mse_loss(seq_pred_stu[unlabel_mask], seq_pred_tch[unlabel_mask].detach())
            cons_mm_loss = F.mse_loss(mm_pred_stu[unlabel_mask], mm_pred_tch[unlabel_mask].detach())
            cons_loss = cons_seq_loss + cons_mm_loss
            ...
            # Update EMA
            student, teacher = update_ema(config.exp_settings.exp_params.ema_factor, 
                                          optim_count, student, teacher)
...
def update_ema(config, epoch, global_step, model, ema_model): # student, teacher
    if epoch < config.exp_settings.mean_teacher.rampup_length:
        pass
    else:
        config.exp_settings.mean_teacher.ema_factor = 0.999
    
    alpha = min(1 - 1 / (global_step + 1), config.exp_settings.mean_teacher.ema_factor)
    for ema_params, params in zip(ema_model.parameters(), model.parameters()):
        ema_params.data.mul_(alpha).add_(params.data, alpha=1 - alpha)
    return config, model, ema_model

def save_student_teacher_both(config, student: torch.nn.Module, teacher: torch.nn.Module):
    # student_path = config.datapath.checkpoint_path / f"Best_student_{config.data_preprocess.val_fold_num}.pt"
    student_path = config.datapath.checkpoint_path / "Best_student.pt"
    teacher_path = config.datapath.checkpoint_path / "Best_teacher.pt"
    torch.save(student.state_dict(), student_path)
    torch.save(teacher.state_dict(), teacher_path)

def load_student_teacher_both(config, model_name: str):
    student, teacher = eval(model_name)(), eval(model_name)()
    # student.load_state_dict(torch.load(config.datapath.checkpoint_path / f"Best_student_{config.data_preprocess.val_fold_num}.pt"))
    student.load_state_dict(torch.load(config.datapath.checkpoint_path / "Best_student.pt"))
    teacher.load_state_dict(torch.load(config.datapath.checkpoint_path / "Best_teacher.pt"))
