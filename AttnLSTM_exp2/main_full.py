...
from metric import decide_murmur_with_threshold, compute_cross_val_weighted_murmur_accuracy
...
def main_full(config, tune_dict=None):
    ...
    # Fit with all Train_data
    models_dict = train_model_full(config, val_fold_results_dict["stop_epoch"], train_dataset, unlabeled_dataset)
    
    test_murmur_wma_stu = test(config, df_dict["test_patient_df"], df_dict["test_recording_df"], 
                            config.datapath.test_data, models_dict["student"], optim_thr_stu)   
    
    test_murmur_wma_tch = test(config, df_dict["test_patient_df"], df_dict["test_recording_df"], 
                               config.datapath.test_data,  models_dict["teacher"], optim_thr_tch)
...
if __name__ == '__main__':
    ...
    config_result = main_full(config, tune_dict= None)
