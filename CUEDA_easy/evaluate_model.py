...
from utils import load_patient_data, get_murmur, get_outcome, load_challenge_outputs, compare_strings


# Evaluate the models.
def evaluate_model(label_folder, output_folder): # test_data_csv파일들 경로, prediction_csv파일들 경로
    # Define murmur and outcome classes.
    murmur_classes = ['Present', 'Unknown', 'Absent']
    murmur_classes_wo_unknown = ['Present', 'Absent']
    ...
    murmur_labels_wo_unknown = murmur_labels[:, [0, -1]]
    
    murmur_binary_outputs = enforce_positives(murmur_binary_outputs, murmur_classes, 'Present')
    murmur_binary_outputs_wo_unknown = murmur_binary_outputs[:, [0, -1]]
    ...
    murmur_weighted_accuracy_wo_unknown = compute_weighted_accuracy_wo_Unknown(murmur_labels_wo_unknown, murmur_binary_outputs_wo_unknown, murmur_classes_wo_unknown)
    ...
    murmur_scores = (murmur_classes, murmur_auroc, murmur_auprc, murmur_auroc_classes, murmur_auprc_classes, \
        murmur_f_measure, murmur_f_measure_classes, murmur_accuracy, murmur_accuracy_classes, murmur_weighted_accuracy, murmur_weighted_accuracy_wo_unknown, murmur_cost)
...
def compute_weighted_accuracy_wo_Unknown(labels, outputs, classes):
    # Define constants.
    if classes == ['Present', 'Absent']:
        weights = np.array([[5, 1], [5, 1]])
    elif classes == ['Abnormal', 'Normal']:
        weights = np.array([[5, 1], [5, 1]])
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test_pred_folder", type=str, default= None) 
    args = parser.parse_args()
    
############################################################################################################################################################  
############################_Assign test_data and test_pred_path, __########################################################################################

    args.test_data_folder = '/Data2/murmur/test'
    args.test_pred_folder = '/Data1/hmd2/notebooks_th/CUEDA_easy/models/2024-10-03_18:21:09/result_wo_tree'
...
############################################################################################################################################################   
    ...
    murmur_scores, outcome_scores = evaluate_model(args.test_data_folder, args.test_pred_folder)

    classes, auroc, auprc, auroc_classes, auprc_classes, f_measure, f_measure_classes, accuracy, accuracy_classes, weighted_accuracy, murmur_weighted_accuracy_wo_unknown, cost = murmur_scores
    murmur_output_string = 'AUROC,AUPRC,F-measure,Accuracy,Weighted Accuracy,Weighted Accuracy_wo_Unknown,Cost\n{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}\n'.format(auroc, auprc, f_measure, accuracy, weighted_accuracy, murmur_weighted_accuracy_wo_unknown, cost)
    murmur_class_output_string = 'Classes,{}\nAUROC,{}\nAUPRC,{}\nF-measure,{}\nAccuracy,{}\n'.format(
        ','.join(classes), # Present,Unknown,Absent
