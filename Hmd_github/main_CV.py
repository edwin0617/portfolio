...
from dataset import config, load_dataframes, Stronglabeled_Dataset, Unlabeled_Dataset
from train_validate_test import train_validate_model, merge_df, test, test_CV
from metric import*
...
def main(config, tune_dict= None):
    ...
    # config = tune_hparams(config, tune_dict) # Tune hparams
    config = check_config_path(config)
    config = make_checkpoint_dirs(config)

    set_seed(config.exp_params.random_seed) # Set seed_number
    set_one_gpu(config.virtual_settings.gpu_index) # Set 1 gpu    
    ...
    for fold in range(config.dataset.num_k):
        
        config.dataset.val_fold_num = fold
        ...
        cv_models_stu[f"model_{fold}"] = val_fold_results_dict["student"]
        cv_models_tch[f"model_{fold}"] = val_fold_results_dict["teacher"]
        
        cv_results_dict_stu.update(val_fold_results_dict["student_fold_preds"])
        cv_results_dict_tch.update(val_fold_results_dict["teacher_fold_preds"])
    
    
    # Student
    rec_predictions_df_stu = pd.DataFrame.from_dict(cv_results_dict_stu, orient="index")
    recording_df_stu = df_dict["recording_df"].merge(rec_predictions_df_stu, left_index=True, right_index=True)    
    
    # Teacher
    rec_predictions_df_tch = pd.DataFrame.from_dict(cv_results_dict_tch, orient="index")
    ...
    _ = compute_cross_val_weighted_murmur_accuracy(val_murmur_preds, print= True)  
    print(f"Val_WMA_stu: {val_murmur_wma_stu}\n")

    # test_preds_dict_stu = {}
    
    # for _, student_fold in cv_models_stu.items():
    
    test_murmur_wma_stu = test_CV(config, df_dict["test_patient_df"], df_dict["test_recording_df"], 
                            config.datapath.test_data, cv_models_stu, optim_thr_stu)   
    print(f"Test_WMA_stu: {test_murmur_wma_stu}\n")
        # test_preds_dict_stu.update(test_murmur_wma_stu)
    ...
    test_murmur_wma_tch = test_CV(config, df_dict["test_patient_df"], df_dict["test_recording_df"], 
                        config.datapath.test_data, cv_models_tch, optim_thr_tch)  
