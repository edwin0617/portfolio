...
def train_validate_model(config, df_dict, train_dataset, unlabeled_dataset, val_dataset):
    ...
    optim = torch.optim.AdamW(student.parameters(), 
                            ...
                            weight_decay=1e-4, betas=(0.9, 0.999), eps=1e-8) #  weight_decay=1e-2, betas=(0.9, 0.999), eps=1e-8)
    ...
    for epoch in tqdm(range(1, config.exp_params.total_epoch + 1)):
        ...
        for i, (batch_strong, batch_unlabel) in enumerate(zip(train_dataloader, cycle(unlabeled_dataloader))):
            ...
            total_loss = (seq_loss + murmur_loss)  + (cons_loss * consis_weight)
            
            train_total_losses += total_loss.cpu().item()
            train_seq_losses += seq_loss.cpu().item()
            train_mm_losses += murmur_loss.cpu().item()
            cons_losses += cons_loss.cpu().item()
        ...
        with torch.no_grad():
            ...
            if val_mm_losses < best_murmur_loss:
                ...
                save_student_teacher_both(config.datapath.checkpoint_path, student, teacher)
                ...
                # what if do not use Early-stopping method...??
                # pass

            exp_scheduler.step(val_mm_losses)
    ...
    student, teacher = load_student_teacher_both(config.datapath.checkpoint_path, model_name)
...
def save_student_teacher_both(model_folder:str, student: torch.nn.Module, teacher: torch.nn.Module):
...
def load_student_teacher_both(checkpoint_path: pathlib.Path, model_name: str):
    student, teacher = eval(model_name)(), eval(model_name)()
    student.load_state_dict(torch.load(checkpoint_path / "Best_student.pt"))
    teacher.load_state_dict(torch.load(checkpoint_path / "Best_teacher.pt"))
